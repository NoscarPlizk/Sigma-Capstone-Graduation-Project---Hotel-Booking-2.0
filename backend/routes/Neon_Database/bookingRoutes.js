...
export function registerBookingRoutes(app, { pool, stripe }) {
  app.post("/api/start-setting-registry-data-in-db", async (req, res) => {
    const client = await pool.connect();
    let transactionStarted = false;

    try {
      const { firebaseUser, bookingRegistry, stripePaymentIntentId } = req.body;
      ...
      if (
        ...
        if (existingBooking) {
          return res.json({
            ...
            bookingCode: existingBooking.booking_code,
            firebaseUid:
              existingBooking.firebase_uid ?? req.body?.firebaseUser?.firebase_uid,
  ...
  app.get(
    "/api/booking-records/by-payment-intent/:paymentIntentId",
    async (req, res) => {
      const client = await pool.connect();
